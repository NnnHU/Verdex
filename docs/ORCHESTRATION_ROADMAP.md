...
| 5 ASR 数据清洗（可选） | ⬜ **下一步（可选）** | — |

**核心 5 阶段（1a/2/3/4）全部完成**。阶段 5（ASR 清洗）为可选项，按需做。详细进度与实现细节见 [HANDOFF.md](./HANDOFF.md) 第四节。
...
#### Map-Reduce 加速方案（按可行性排序）

**A. 立即可做（配置级，不改代码）**
1. **Reduce 单独换更快模型**：Map/Reduce 现在都用 DeepSeek V3。若硅基流动有更快变体或 Qwen 等，Reduce 单独用它（Reduce 输入大，模型速度差异放大）。
2. **精简 Map 输出再 Reduce**：Map 让模型只抽"精简版"（每条目少几个字段），Reduce 输入小 → 快。代价：细节减少。

**B. 需要改代码（中等改动，推荐）**
3. **Reduce 分批合并（树形 Reduce）** ⭐：N 份不一次性合，而是两两合并：
   ```
   7 份 → JSON1+2→A, 3+4→B, 5+6→C, 7→D（4 个并行小合并）
        → A+B→X, C+D→Y（2 个并行小合并）
        → X+Y→最终（1 次小合并）
   ```
   每步输入小（只合 2 份），速度快；部分步骤并行；且**不会单次超时**（大调用的超时风险分散到多个小调用）。这是大数据 Map-Reduce 的标准优化（Hadoop/Spark 都这么做）。
   - 代价：合并质量可能略降（分批合 vs 一次看全）；改动较大（runMapReduce 重构 + 多轮合并调度）。
   - 收益：Reduce 从"1 次 2-3 分钟大调用"变"多轮各几秒小调用"，总时长通常更短且稳定。

**C. 结构性限制（无法突破）**
4. **Reduce 的串行本质**：合并必须串行，是 Map-Reduce 范式固有特性。树形 Reduce 只是"把一次大串行拆成多次小串行 + 部分并行"，没消除串行，但显著降低单次负担。

**当前判断（2026-07-24）**：2-3 分钟主要是 Reduce 串行开销，属正常。体验三件套（计时/Stop/进度）已让等待可观测。**树形 Reduce 是最值的优化，但非必须**——若当前可接受，先不动；若要提速，优先做树形 Reduce。
