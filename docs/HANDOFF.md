...
**实测结果（2026-07-24，g1.txt 格兰瑟姆播客稿）**：
- **清洗 API 正常**：后台 scripts/test-clean.mjs 实测，清洗 g1.txt 前 2000 字耗时 45s，繁体→简体、实体归一化正确（"格蘭色母"→"格兰瑟姆"、"波士頓"→"波士顿"）。全文 12323 字符清洗约 60-90s（一次完整 API 调用：读全文+改错+输出全文）。
- **前端验证**：勾选「清洗」+ 挂 g1.txt → chip 显示「清洗中…」→ 约 1 分钟后变「已清洗」→ 用 Extract + 思维模型库 schema 提取 → 产出 JSON 实体名全部正确（"均值回归""凯恩斯法则""REITs"，无乱码），年份正确（1991/1997/1994，无"208年"误识），全简体统一。
- **清洗中发送逻辑（修复后）**：清洗未完成时发送会等待（顶部提示「等待 N 个附件清洗完成…」），完成后自动用清洗版文本发送，不会用脏文本。

**实测后修复（2026-07-24）**：
1. **cleanAttachment 闭包时序 bug**：cleanAttachment 从闭包 sessions 读附件，但 addAttachments 刚更新 sessions 还没反映到闭包 → 找不到新附件 → 不清洗。修复：cleanAttachment 接受可选 sourceText 参数，handleAddFiles 直接传 a.text。
